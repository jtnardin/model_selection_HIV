...
switch ode_solve
    ...
    for i = 1:length(c)
        for j = 1:length(delta)
            ...
            J(i,j) = cost_function_model1(q,qknown,tdata,ydata,ode_solve,h(tol_ind));
        end
    end
    ...
    if save_fig == 1
        exportfig(gcf,['cost_contour_pat_' num2str(patient) '_' ode_solve '_'...
            num2str(tol_ind) '.eps'],'color','rgb','fontsize',1.5)
        saveas(gcf,['cost_contour_pat_' num2str(patient) '_' ode_solve '_'...
            num2str(tol_ind) '.fig'])
    end